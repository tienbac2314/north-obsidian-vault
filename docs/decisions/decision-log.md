...
- Exclusions: Remove Importer from enabled runtime after Notion verification. Do not add Lazy Loader, Dataview, Datacore, Tasks, Templater, QuickAdd, Meta Bind, JS Engine, Charts, Todoist, Iconic, Style Settings, Hider, Notebook Navigator, or Omnisearch.
...
- Rollback: Point Homepage to distinct `SYSTEM/Core Hub/Home Core.md`, preserve a timestamped enabled-plugin list, and keep only FNS, Homepage, and Custom Sort enabled. Exact switch and restore commands live in the [vault operating guide](../../vault-template/SYSTEM/Guides/vault-operating-guide.md#small-rich-runtime-rollback). Rich files remain untouched. If this fails, restore a verified offline checkpoint after separately preserving current state. Rollback never adds another sync engine.

## DEC-038: Contain personal-vault promotion exception

- Status: Accepted corrective decision after independent Phase 2 review.
- Trigger: User explicitly required the complete existing Notion import inside the FNS-backed `G:\Obsidian` vault and requested eventual Windows/Android use. FNS is whole-vault transport. The 164-file Notion tree therefore crossed DEC-035's synthetic-only boundary before the formal Release 1B gate was recorded.
- Decision: Treat the existing FNS-backed vault as a bounded human-only personal-data promotion exception. Preserve current bytes and normal note/attachment sync; do not attempt destructive rollback or pretend the synthetic boundary still holds. This exception does not prove Release 1B, Android parity, independent recurring backup, or recovery readiness.
- Immediate controls: Configuration Sync is disabled and verified persistent on Windows. FNS MCP, external REST consumers, sharing, Git automation, mirror, Cloud Preview deletion, Hermes, and external writers remain disabled. No additional batch import or more sensitive personal/employer content enters until physical Android smoke testing, Android Configuration Sync disablement, and independent backup policy pass.
- Executable-note boundary: DataviewJS is enabled vault-wide because Dataview has no path allowlist. Only the reviewed Home block and three reviewed HUB Datacore JSX blocks are accepted. Imported, downloaded, synchronized, or agent-written notes must be scanned for executable fences before Reading view.
- Recovery: Final completion checkpoint `G:\Obsidian Backups\phase-2-dusk-complete-20260724-141701` contains 409 files and 84,267,967 bytes. Source and backup manifests have zero differing rows and SHA-256 `716C45484B445A6252CF0A2AD391F2524EAC0990525E7D3378408705503E9D74`. It is outside the synchronized vault and repository.
- Remote configuration: Earlier Windows work unintentionally enabled FNS Configuration Sync. Disabling it stops further Windows configuration propagation but does not prove remote configuration rows are absent or Android has the toggle disabled. Before Android reconnects, verify its toggle is off. Clear remote configuration only through a separate controlled recovery test; do not improvise deletion against the only mobile setup.
- Supersedes: DEC-035 synthetic-only boundary only for bytes already present in this named human vault. All other DEC-035 restrictions remain accepted. DEC-037 is amended so FNS configuration hash may change only for this verified disablement.
- Evidence: [Dusk runtime inventory](../research/2026-07-24-dusk-runtime-inventory.md) and [Phase 2 implementation evidence](../research/2026-07-24-phase-2-implementation-evidence.md).
